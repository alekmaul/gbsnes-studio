...
class Scene extends Component {
  ...
  render() {
    ...
    // Game Boy scenes are limited by a shared BG/OBJ sprite-frame VRAM budget;
    // the SNES engine has a fixed OBJ sheet and is instead limited by the number
    // of distinct actor sprite sheets loaded at once (targets/*.js).
    const targetDescriptor = getTarget(target);
    const spriteSheetLimited = targetDescriptor.maxSpriteSheets != null;
    const spriteLabel = spriteSheetLimited ? "S" : "F";
    const spriteUsed = spriteSheetLimited ? spriteSheetCount : frameCount;
    const spriteMax = spriteSheetLimited
      ? targetDescriptor.maxSpriteSheets
      : targetDescriptor.maxSpriteFrames;
      ...
      >
        ...
        <div className="Scene__Info" onMouseDown={this.onStartDrag}>
          ...
          <span
            title={
              spriteSheetLimited
                ? `Distinct actor sprite sheets in this scene (${spriteUsed}). ` +
                  `The SNES engine loads at most ${spriteMax} across the whole project; ` +
                  `the extra sheets fall back to the first sheet's tiles/palette.`
                : `Number of frames used by actors in scene. ${
                    spriteUsed <= spriteMax
                      ? `This scene has used ${spriteUsed} of ${spriteMax} available.`
            ...
            className={cx({
              "Scene__Info--Warning": spriteUsed === spriteMax,
              "Scene__Info--Error": spriteUsed > spriteMax
            })}
          >
            {spriteLabel}: {spriteUsed}/{spriteMax}
          </span>
          {"\u00A0 \u00A0"}
          <span
...
Scene.propTypes = {
  ...
  spriteSheetCount: PropTypes.number.isRequired,
...
function mapStateToProps(state, props) {
  ...
  return {
    ...
    spriteSheetCount: getSceneUniqueSpriteSheets(state, props).filter(Boolean)
      .length
  };
}
