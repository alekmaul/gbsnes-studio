...
const wrapLimits = (hasAvatar) => {
  return {
    maxPerLine: hasAvatar ? target.maxTextLineCharsWithAvatar : target.maxTextLineChars,
    maxTotal: hasAvatar ? target.maxTextTotalCharsWithAvatar : target.maxTextTotalChars
  };
};
...
const fields = [
  {
    ...
    updateFn: (string, field, args) => {
      const { maxPerLine, maxTotal } = wrapLimits(!!args.avatarId);
