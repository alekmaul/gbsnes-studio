...
const buildUiGfx = async uiDir => {
  ...
  // v2: Array.from(), not [...seen.values()] - this codebase's babel target
  // compiles array spread via a __spreadArrays helper that assumes every
  // spread source has a real .length property (fine for arrays/array-likes,
  // but a Map iterator has none) - `[...iterator]` throws "Invalid array
  // length" here (NaN accumulated from an undefined .length). Array.from()
  // consumes any iterable directly and isn't subject to that helper at all.
  const byLum = Array.from(seen.values()).sort((a, b) => lum(b.c) - lum(a.c));
  let palColours;
  if (byLum.length <= 3) {
