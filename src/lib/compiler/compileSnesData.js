 ...
 *
 * v2: Engine Fields (topdown_grid, the Platformer physics constants) are
 * NOT wired to real per-project values here - appData/src/snes/ has no
 * engine.json yet (no Settings-page UI to author per-project overrides
 * either, that's M9 territory), so the SNES engine keeps its M5-era
 * hardcoded-default globals unchanged. GB Studio 2.0's real Engine Field
 * pipeline (precompileEngineFields/compileEngineFields, engineFieldValues)
 * is a genuine, separate design/implementation effort for a later milestone,
 * not something to half-wire here.
 *
