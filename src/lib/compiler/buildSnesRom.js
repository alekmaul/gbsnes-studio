...
/*
 * SNES build orchestration (PVSnesLib), M8.
 *
 * The Game Boy target shells out to `lcc` through a generated make.bat
 * (buildMakeBat.js) via makeBuild.js. PVSnesLib normally builds through a
 * GNU-make file (`snes_rules`) which needs make + a Unix shell + coreutils.
 * To avoid shipping those, this module drives the four tools directly, the
 * same way snes_rules does:
 *
 ...
 * toolchain) from the M0 cadrage. Ported forward from the v1.1.4 SNES port
 * (main branch) with no functional changes - resolvePvsHome()'s asar fixes
 * (v1.1.2/v1.1.3) already apply cleanly since helpers/fsCopy.js's asar-safe
 * pathExists() was re-added in M2, ahead of this port.
 */
...
const buildSnesRom = async ({
  ...
  const linkfile = ["[objects]"].concat(objFiles).concat(libObjs).join("\n");
