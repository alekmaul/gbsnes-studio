...
// Regression: a real packaged build of the app threw "PVSnesLib toolchain
// not found" even though the toolchain genuinely shipped inside app.asar.
// Root cause: fs-extra's pathExists()/fs.access() is NOT one of the fs
// functions Electron's asar support patches (stat/lstat/readdir/readFile/
// createReadStream are; access/accessSync famously aren't) - so
...
// This test only proves the plain-filesystem true/false logic, since jest
// runs under plain Node, not a packaged Electron asar - the asar-
// transparency itself is exactly what distinguishes pathExists() (broken)
// from the fs.lstat() this helper uses instead.
...
// Regression (ported from the same fix on `main`): a packaged Linux build's
// "Build ROM" failed with "spawn .../smconv EACCES" - copyFile() wrote the
// destination with Node's default stream mode (0o666, no executable bit)
// regardless of the source file's own permissions, so every file this
// helper copies (a vendored toolchain extracted from app.asar on
// Linux/macOS, in particular) lost its executable bit and could no longer
// be spawned. Only meaningful on a platform with real POSIX permission bits
// (skipped on Windows, where there's no executable-bit concept at the
// filesystem level to preserve).
...
maybeOnPosix("fsCopy - copy preserves executable permissions", () => {
  ...
  // makeBuild.js / ensureBuildTools.js, all extracting a toolchain packed
  // inside app.asar) must win outright, not just supply a fallback default
  // - trusting the source file's own reported mode for a path inside
  // app.asar isn't something to rely on blindly.
