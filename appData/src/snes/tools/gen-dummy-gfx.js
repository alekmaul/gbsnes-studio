...
const scene1 = [1, 0, 0, EV_SCENE1, SCENE1_W, SCENE1_H, ...scene1Collision];
...
(async () => {
  ...
  // compiler (compileSnesData.js). sprite_pal_for_slot[] maps slot -> OBJ
  // palette (1 and 2 reserved for emotes / avatars).
  const sprPal = new Array(8 * 32).fill(0);
  fixed.spritePaletteBytes.slice(0, 32).forEach((b, i) => {
    sprPal[i] = b;
  });
  const spritePalForSlot = [0, 3, 4, 5, 6, 7, 0, 0];
  const avatarPal = fixed.spritePaletteBytes; // OBJ palette 2, one 16-colour pal
...
extern const unsigned char spr_pal[${sprPal.length}];
extern const unsigned char emote_pal[${emotePal.length}];
extern const unsigned char avatar_pal[${avatarPal.length}];
...
#define AVATAR_PAL_SIZE ${avatarPal.length}
...
${cArray("spr_pal", sprPal)}
${cArray("emote_pal", emotePal)}
${cArray("avatar_pal", avatarPal)}
${cArray("ui_font", uiFont)}
${cArray("ui_pal", uiPal)}
${cArray("sprite_slot_for_index", [0])}
${cArray("sprite_type_for_slot", [SPRITE_STATIC, 0, 0, 0, 0, 0, 0, 0])}
${cArray("sprite_frames_for_slot", [1, 1, 1, 1, 1, 1, 1, 1])}
