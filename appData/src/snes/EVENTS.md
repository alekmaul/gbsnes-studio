...
| Actor/Player: Set Sprite State | ✅ | v4, "named animation states" (user request: GB Studio 3.2.1's per-sprite Animations panel). A sprite sheet can carry an optional named `states[]` list, each just a name + pointer to another already-existing sheet - no per-tile composition, unlike B's real metasprite system (out of scope, see the plan's rationale: the SNES OBJ sheet is a fixed, already-fully-allocated 256-tile budget, so composing bigger sprites at runtime isn't viable here). `EVENT_ACTOR_SET_STATE`/`EVENT_PLAYER_SET_STATE` are a thin compiler-only layer: `stateId` resolves through the selected sheet's `states[]` to a real target sheet, then calls the exact same `actorSetSprite`/`playerSetSprite` helpers Set Sprite Sheet already uses - no new opcodes, so it inherits that event's own row above verbatim (same "already loaded into this scene" restriction, same fixed `frames_len` recompute). `compileSnesData.js`'s runtime-sprite scanner resolves the same `stateId` indirection, or a state-only sheet would hit the exact 0xff-slot bug documented above. Verified two ways: a jest test compiles `EVENT_PLAYER_SET_STATE` and an equivalent hand-authored `EVENT_PLAYER_SET_SPRITE` through the real `compileEntityEvents`/`ScriptBuilder` pipeline and asserts byte-identical output (proves zero new runtime behaviour); a real toolchain build (an animated 6-frame sheet with one named state pointing at a 3-frame sheet, switched via `EVENT_PLAYER_SET_STATE` in the start scene's own script) booted in Mesen - WRAM settled at `sprite_type=1, frames_len=1` (the 3-frame sheet's own correct values, not the original 6-frame sheet's stale `frames_len=2`), stable for 130+ frames. The reducers/events/tests are unchanged and still work from script; the Sprites page's own "States" panel was removed from view shortly after shipping (user-found: not useful positioned in the left dock) - `SpriteStatesEditor.tsx` still exists, just unmounted from `SpritesPage.js`. |
| Sprites page: Animations / Frames panel (editor only, no opcode) | N/A | v4 follow-up, same user request. `getSpriteAnimations(type, numFrames)` (`src/lib/helpers/spriteAnimations.js`) labels this engine's existing, purely file-width-derived frame convention (down/up/side × idle/walk-cycle-AB, or a single flat "Idle" list for a non-directional sheet) - a pure, read-only function, no new compiler/engine behaviour. The Sprites page shows it as an "ANIMATIONS" list (left dock) + a "Frames: `<name>`" strip docked over the right-hand preview area (`SpriteAnimationsPanel.tsx`/`SpriteFramesPanel.tsx`) - user-found the first cut crammed frame thumbnails into the narrow left sidebar; moved next to the actual preview image instead. Each frame box is a real, absolute index (0..`numFrames`-1) into the sheet's own strip via a new `SpriteSheetCanvas` `rawFrame` prop (bypasses the direction-based `directionToFrame` helper entirely). **Deliberately read-only, reverted from a brief editable pass** (a "+"/click-to-remove editor-only frame curation, `SpriteSheet.animationFrames?: number[][]`) - on review it didn't hold up: for a directional (actor/actor_animated) sheet the engine hardcodes exactly 1 idle frame and 2 moving frames (`scene.c`'s `actor_render_tile`, no per-animation frame table), so editing the list there could never do anything the compiled game would honour; for an Idle-only sheet it could only reorder/repeat/drop frames the PNG already has, not add real new content. Removed rather than leave a control implying capability that doesn't exist. Adding a genuinely new named animation (not just curating an existing one's frames) has the same "engine has no data-driven per-animation table" limitation and isn't supported either. |
