...
var needed. `package.json` is 1.1.1.**
...
src/assets.{c,h}   pointer tables + scene blobs + script bytecode + strings — generated, dummy committed
src/data/*.as      one superfree section per background / font / OBJ sheet / OBJ palette
src/data/data.asm  .include "hdr.asm" + one .include per src/data/*.as — the file the build assembles
tools/gen-dummy-gfx.js   regenerates src/assets.* + src/data/* (UI gfx from templates/gbhtml/assets/ui/)
